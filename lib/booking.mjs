...
export async function searchBookingFlights({
  ...
  for (const offer of offers) {
    ...
    results.push({
      origin,
      destination,
      price: typeof price === "string" ? parseFloat(price.replace(/[^\d.]/g, "")) : Math.round(price * 100) / 100,
      ...
      note:
        actualCurrency !== "EUR"
          ? `dati non ufficiali (Booking.com via RapidAPI); prezzo in ${actualCurrency}, non EUR — converti prima di confrontare`
