...
import { buildBookingUrl } from "./bookingLink.mjs";
...
export async function searchBookingFlights({
  ...
  // Il wrapper RapidAPI a volte restituisce HTTP 200 con dentro un errore
  // vero (es. il proxy interno verso booking.com che cade) invece di un
  // vero e proprio "nessun volo trovato". Se non lo distinguiamo, il
  // risultato sparisce silenziosamente senza retry e senza avviso.
  if (json?.data?.errors?.message) {
    throw new Error(
      `Booking.com errore upstream/proxy per ${origin}->${destination}: ${json.data.errors.message}`
    );
  }
  ...
  for (const offer of offers) {
    ...
    // I singoli "leg" spesso non riportano il nome del vettore: la fonte
    // affidabile è priceBreakdown.carrierPriceBreakdown (un elemento per
    // vettore coinvolto — indice 0 = andata, indice 1 = ritorno se sono
    // compagnie diverse, es. Ryanair andata + Wizz Air ritorno).
    const carrierBreakdown = offer.priceBreakdown?.carrierPriceBreakdown || [];
    const carrierFallbackOut = carrierBreakdown[0]?.marketingCarrierName || offer.priceBreakdown?.carrierTaxBreakdown?.[0]?.carrier?.name || null;
    const carrierFallbackRet = carrierBreakdown.length > 1 ? carrierBreakdown[1]?.marketingCarrierName || null : carrierFallbackOut;

    const airlineOut = firstLegOut.carrierName || firstLegOut.airline || offer.airline?.name || carrierFallbackOut;
    const flightNumOut = firstLegOut.flightNumber || firstLegOut.flightNo || null;
    const airlineRet = legsRet ? legsRet[0]?.carrierName || legsRet[0]?.airline || carrierFallbackRet : null;
    ...
    results.push({
      ...
      source: "booking",
      conversionFailed: !converted && actualCurrency !== "EUR",
      note: converted
