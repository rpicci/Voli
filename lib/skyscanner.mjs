...
export async function resolveSkyId({ apiKey, iataCode, cache }) {
  ...
  // I campi skyId/entityId non sono sempre al primo livello: molte di
  // queste API (che replicano la struttura interna di Skyscanner) li
  // nidificano sotto navigation.relevantFlightParams. Proviamo entrambi
  // i percorsi.
  const match =
    results.find((r) => (r.skyId || "").toUpperCase() === iataCode.toUpperCase()) ||
    results.find(
      (r) => (r.navigation?.relevantFlightParams?.skyId || "").toUpperCase() === iataCode.toUpperCase()
    ) ||
    results[0];
  ...
  const resolved = {
    skyId: match.skyId || match.navigation?.relevantFlightParams?.skyId || null,
    entityId: match.entityId || match.navigation?.relevantFlightParams?.entityId || null,
  };

  if (!resolved.skyId || !resolved.entityId) {
    console.log(
      `[skyscanner DEBUG struttura searchAirport per ${iataCode}] ` +
        JSON.stringify(match).slice(0, 800)
    );
  }
...
export async function searchSkyscannerFlights({
  ...
  console.log(
    `[skyscanner DEBUG aeroporti risolti] ${origin}->skyId=${originIds.skyId},entityId=${originIds.entityId} ` +
      `${destination}->skyId=${destinationIds.skyId},entityId=${destinationIds.entityId}`
  );
  ...
  let itineraries = json?.data?.itineraries || json?.itineraries || [];

  // Risposta "a sessione": la prima chiamata avvia la ricerca (context con
  // sessionId, status "complete" ma senza itinerari). Riproviamo la stessa
  // richiesta aggiungendo il sessionId, fino a 2 volte, con una breve pausa.
  const sessionId = json?.data?.context?.sessionId || null;
  if ((!Array.isArray(itineraries) || itineraries.length === 0) && sessionId) {
    for (let attempt = 0; attempt < 2 && itineraries.length === 0; attempt++) {
      await new Promise((resolve) => setTimeout(resolve, 1500));
      const retryUrl = new URL(url.toString());
      retryUrl.searchParams.set("sessionId", sessionId);
      const retryRes = await fetchWithRetry(retryUrl.toString(), { headers: headers(apiKey) });
      if (!retryRes.ok) break;
      json = await retryRes.json();
      itineraries = json?.data?.itineraries || json?.itineraries || [];
    }
  }
  ...
  if (!Array.isArray(itineraries) || itineraries.length === 0) {
    ...
    console.log(
      `[skyscanner DEBUG nessun itinerario dopo retry con sessionId ${origin}->${destination}] chiavi json: [${topLevelKeys}] ` +
