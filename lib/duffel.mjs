...
// di volo). NON copre Ryanair (vende solo diretto) e la copertura Wizz Air
// non è garantita su tutte le rotte — per queste serve Travelpayouts.
...
export async function searchFlights({
  ...
  if (apiKey && apiKey.startsWith("duffel_test_")) {
    throw new Error(
      "DUFFEL_API_KEY è una chiave di TEST (duffel_test_...): restituisce voli finti " +
        "('Duffel Airways' e altri dati simulati), non ricerche reali. Serve una chiave " +
        "live (duffel_live_...) da dashboard.duffel.com → Developers."
    );
  }
  ...
  for (const offer of offers) {
    ...
    if (returnSlice) {
      ...
      if (!inTimeWindow(returnFirstSeg.departing_at, arriveTimeFrom, arriveTimeTo)) continue;
    ...
    results.push({
      ...
      returnDepartTime: returnFirstSeg ? returnFirstSeg.departing_at.slice(11, 16) : null,
