...
// L'API restituisce data/ora nel formato "YYYY-M-D HH:MM", con mese e
// giorno SENZA zero iniziale quando sono a una cifra (es. "2026-8-7 07:00").
// Non possiamo quindi tagliare per posizione fissa: separiamo su spazio e
// normalizziamo esplicitamente ogni componente.
function splitRawDateTime(raw) {
...
function inTimeWindow(rawTime, from, to) {
  const { time: hhmm } = splitRawDateTime(rawTime);
  if (!hhmm) return true;
...
export async function searchGoogleFlights({
  ...
  // della fonte stessa.
  console.log(
    `[googleflights DEBUG stops ${origin}->${destination}] finestra oraria ricevuta: partenza[${departTimeFrom||'-'}—${departTimeTo||'-'}] arrivo[${arriveTimeFrom||'-'}—${arriveTimeTo||'-'}] round-trip:${!!returnDateFrom} ` +
      JSON.stringify(
        itineraries.slice(0, 5).map((it) => {
          ...
          return {
            ...
            flight_numbers: legs.map((l) => l.flight_number),
            price: it.price,
            depart_time_raw: legs[0]?.departure_airport?.time || null,
            arrive_time_raw: legs[legs.length - 1]?.arrival_airport?.time || null,
          };
        })
      )
  );
  ...
  for (const it of itineraries) {
    ...
    // Frase in inglese ("Flights from X to Y on DATA"), non in italiano:
    // è il formato standard che Google riconosce come trigger per il box
    // voli/app Flights, indipendentemente dalla lingua dell'interfaccia.
    const searchQuery = returnDateFrom
      ? `Flights from ${origin} to ${destination} on ${departDateFrom} returning ${returnDateFrom}`
      : `Flights from ${origin} to ${destination} on ${departDateFrom}`;
    const bookingUrl = `https://www.google.com/travel/flights?q=${encodeURIComponent(searchQuery)}`;
    ...
    results.push({
      ...
      currency,
      departDate: splitRawDateTime(departTime).date || departDateFrom,
      departTime: splitRawDateTime(departTime).time,
      arriveTime: splitRawDateTime(arriveTime).time,
      ...
      try {
        ...
        if (Array.isArray(returnItineraries) && returnItineraries.length > 0) {
          ...
          const matchesFilters = (r) => {
            ...
            const firstLeg = rLegs[0];
            const departTime = firstLeg?.departure_airport?.time || null;
            const timeOk = inTimeWindow(departTime, arriveTimeFrom, arriveTimeTo);
          ...
          if (returnLegs.length > 0) {
            ...
            cheapest.returnDepartTime = splitRawDateTime(retFirstLeg.departure_airport?.time).time;
            cheapest.returnArriveTime = splitRawDateTime(retLastLeg.arrival_airport?.time).time;
