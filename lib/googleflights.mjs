    ...
    if (cheapest._nextToken) {
      try {
        ...
        if (Array.isArray(returnItineraries) && returnItineraries.length > 0) {
          ...
          // Tra le opzioni di ritorno già ricevute (nessuna chiamata in
          // più), scegliamo la più economica TRA QUELLE che rispettano la
          // fascia oraria di arrivo richiesta — non la più economica in
          // assoluto, altrimenti il filtro orario viene ignorato sul ritorno.
          const withinTimeWindow = returnItineraries.filter((r) => {
            const rLegs = r.flights || r.legs || [];
            const lastLeg = rLegs[rLegs.length - 1];
            const arriveTime = lastLeg?.arrival_airport?.time || null;
            return inTimeWindow(arriveTime, arriveTimeFrom, arriveTimeTo);
          });
          const candidatePool = withinTimeWindow.length > 0 ? withinTimeWindow : returnItineraries;
          const noneMatchedWindow = withinTimeWindow.length === 0;
          ...
          if (returnLegs.length > 0) {
            ...
            cheapest.note = noneMatchedWindow
              ? "⚠️ nessun ritorno trovato nella fascia oraria richiesta: mostrato il più economico disponibile, fuori fascia"
              : "dati non ufficiali (scraping Google Flights, quota gratuita limitata), verificare disponibilità reale prima di prenotare";
          }
        }
      } catch (err) {
