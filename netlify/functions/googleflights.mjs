...
export async function searchGoogleFlights({
  ...
  for (const it of itineraries) {
    ...
    // Il booking_token dell'API non è nel formato interno "tfs" che Google
    // si aspetta, quindi il link generato puntava alla home page invece che
    // a un volo specifico. Costruiamo invece un link di RICERCA generico
    // (stessa rotta e date), meno preciso ma sempre funzionante, sia per
    // sola andata che andata+ritorno.
    const searchQuery = returnDateFrom
      ? `Flights from ${origin} to ${destination} on ${departDateFrom} through ${returnDateFrom}`
